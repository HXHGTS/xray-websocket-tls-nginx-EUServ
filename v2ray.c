...
Menu:UI();
    ...
    if (mode == 1) {
        ...
        fclose(config);
        system("curl https://cdn.jsdelivr.net/gh/HXHGTS/xray-websocket-tls-nginx-EUServ/default.conf >> /etc/nginx/conf.d/default.conf");
        system("systemctl restart nginx");
        QRCodeGen();
        printf("正在检测xray与nginx运行状态，以下输出不为空则运行正常！\n");
        printf("--------------以下输出不为空则xray运行正常------------------\n");
...
int install_xray() {
    ...
    system("cp -rf /root/2.pem /usr/local/etc/xray/private.pem");
    printf("正在生成配置文件. . .\n");
    system("curl https://cdn.jsdelivr.net/gh/HXHGTS/xray-websocket-tls-nginx-EUServ/config.json.1 > /usr/local/etc/xray/config.json");
    printf("正在生成UUID. . .\n");
    ...
    fclose(config);
    system("curl https://cdn.jsdelivr.net/gh/HXHGTS/xray-websocket-tls-nginx-EUServ/default.conf >> /etc/nginx/conf.d/default.conf");
    system("wget https://cdn.jsdelivr.net/gh/HXHGTS/xray-websocket-tls-nginx-EUServ/html.zip -O /usr/share/nginx/html/html.zip");
